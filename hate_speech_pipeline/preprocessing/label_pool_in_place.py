...
def main():
    ...
    ap.add_argument(
        "--table",
        default="train_lex_pool",
        help="Pool table name (default train_lex_pool)",
    )
    args = ap.parse_args()
    ...
    hate_idx = next(
        (
            ...
            if "HATE" in str(lab).upper() and "NOT" not in str(lab).upper()
        ),
    ...
    while True:
        ...
        model_ids = [
            cid for cid, t in zip(ids, texts) if t not in ("[deleted]", "[removed]", "")
        ]
        ...
        if model_inputs:
            ...
            updates.extend(
                (1 if p >= THRESHOLD else 0, cid) for cid, p in zip(model_ids, probs)
            )
