...
class DiffusionModerationVisualizer:
    ...
    def plot_thread_analysis(self):
        ...
        bars = ax2.bar(
            pattern_labels, pattern_values, color=["skyblue", "red"], alpha=0.7
        )
        ax2.set_ylabel("Toxicity Score")
        ax2.set_title("Thread Toxicity Patterns")
        ax2.axhline(
            y=0.7, color="orange", linestyle="--", label="High Toxicity Threshold"
        )
        ax2.axhline(
            y=0.5, color="yellow", linestyle="--", label="Medium Toxicity Threshold"
        )
        ax2.legend()
        ...
        if "avg_children_per_thread" in size_distribution:
            ...
            bars = ax4.bar(
                size_labels, size_values, color=["lightgreen", "darkgreen"], alpha=0.7
            )
    ...
    def plot_moderation_impact(self):
        ...
        fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(16, 12))
        fig.suptitle(
            "Diffusion Moderation Impact Analysis", fontsize=16, fontweight="bold"
        )
        ...
        non_zero_actions = [
            (action, count)
            for action, count in zip(action_types, action_counts)
            if count > 0
        ]
        if non_zero_actions:
            ...
            legend_labels = [
                f"{label}: {count} ({count/total_actions*100:.1f}%)"
        ...
        bars = ax3.bar(
            threshold_labels, threshold_values, color="lightcoral", alpha=0.7
        )
        ...
        bars = ax4.bar(
            coverage_labels,
            coverage_counts,
            color=["lightblue", "red", "orange", "yellow"],
            alpha=0.7,
        )
        ...
        for bar, count in zip(bars, coverage_counts):
            ...
            ax4.text(
                bar.get_x() + bar.get_width() / 2.0,
                height + max(coverage_counts) * 0.01,
                f"{count:,}",
                ha="center",
                va="bottom",
                fontsize=9,
    ...
    def plot_comprehensive_risk_matrix(self):
        ...
        fig.suptitle(
            "Comprehensive Risk Assessment Matrix", fontsize=16, fontweight="bold"
        )
        ...
        wedges, texts, autotexts = ax2.pie(
            ...
            startangle=90,
            pctdistance=0.85,
        )  # Move percentages closer to center

        # Create custom legend with counts
        legend_labels = [
            f"{cat}: {count} ({count/risk_counts.sum()*100:.1f}%)"
