...
from diffusion_classification_moderation.diffusion_moderation import DiffusionModerationSystem
from diffusion_classification_moderation.diffusion_analysis import DiffusionAnalysis
from diffusion_classification_moderation.diffusion_moderation_visualisation import DiffusionModerationVisualizer
...
def compute_all_analysis_data(moderator, analyser):
    ...
    # Basic data
    basic_data = {
        'total_comments': len(moderator.merged_data) if moderator.merged_data is not None else 0,
        'high_confidence_comments': len(moderator.get_predictions_by_threshold(HIGH_CONFIDENCE_THRESHOLD)),
        'toxic_comments': len(moderator.get_predictions_by_threshold(BASIC_TOXIC_THRESHOLD))
    }
    ...
    users_to_moderate = moderator.moderate_users(user_hate_threshold=USER_HATE_THRESHOLD, prediction_threshold=MAIN_ANALYSIS_THRESHOLD)
    subreddits_to_moderate = moderator.moderate_subreddits(subreddit_hate_threshold=SUBREDDIT_HATE_THRESHOLD, prediction_threshold=MAIN_ANALYSIS_THRESHOLD)
    ...
    user_behavior_analysis = analyser.analyze_user_behavior_patterns(MAIN_ANALYSIS_THRESHOLD, user_stats)
    subreddit_analysis = analyser.analyze_subreddit_toxicity(MAIN_ANALYSIS_THRESHOLD, subreddit_stats)
    
    # Get hate comments for efficiency analysis
    hate_comments = moderator.merged_data[moderator.merged_data['gcn_prediction'] >= MAIN_ANALYSIS_THRESHOLD] if moderator.merged_data is not None else pd.DataFrame()
    efficiency_analysis = analyser.analyze_moderation_efficiency(MAIN_ANALYSIS_THRESHOLD, hate_comments, bulk_demo_data)
...
def generate_all_visualisations(visualizer, analysis_data):
    ...
    visualizer.generate_all_visualizations(MAIN_ANALYSIS_THRESHOLD, analysis_data['user_stats'], 
                                           analysis_data['subreddit_stats'], analysis_data['subreddit_analysis'], 
                                           analysis_data['comparison'], analysis_data['threshold_analysis'])
...
def run_diffusion_demo(diffusion_data_path):
    ...
    try:
        ...
        display.show_diffusion_data_loaded(diffusion_data_path, len(diffusion_system.data))
