...
class ModerationVisualizer:
    ...
    def plot_threshold_impact(self, threshold_analysis: pd.DataFrame):
        ...
        flagged_06 = threshold_analysis[threshold_analysis['threshold'] == 0.6]['comments_moderated'].iloc[0] if len(threshold_analysis[threshold_analysis['threshold'] == 0.6]) > 0 else 0
        auto_removed_07 = threshold_analysis[threshold_analysis['threshold'] == 0.7]['comments_moderated'].iloc[0] if len(threshold_analysis[threshold_analysis['threshold'] == 0.7]) > 0 else 0
        
        # Comments moderated vs threshold
        ax1.plot(threshold_analysis['threshold'], threshold_analysis['comments_moderated'], 
                marker='o', linewidth=2, markersize=6, color='red')
        ax1.axhline(y=flagged_06, color='orange', linestyle='--', linewidth=2, alpha=0.7, 
                   label=f'Flagged (≥0.6): {int(flagged_06):,}')
        ax1.axhline(y=auto_removed_07, color='darkred', linestyle='--', linewidth=2, alpha=0.7,
        ...
        flagged_06_pct = threshold_analysis[threshold_analysis['threshold'] == 0.6]['percentage_moderated'].iloc[0] if len(threshold_analysis[threshold_analysis['threshold'] == 0.6]) > 0 else 0
        auto_removed_07_pct = threshold_analysis[threshold_analysis['threshold'] == 0.7]['percentage_moderated'].iloc[0] if len(threshold_analysis[threshold_analysis['threshold'] == 0.7]) > 0 else 0
        
        ax2.plot(threshold_analysis['threshold'], threshold_analysis['percentage_moderated'], 
                marker='s', linewidth=2, markersize=6, color='orange')
        ax2.axhline(y=flagged_06_pct, color='orange', linestyle='--', linewidth=2, alpha=0.7,
                   label=f'Flagged (≥0.6): {flagged_06_pct:.2f}%')
        ax2.axhline(y=auto_removed_07_pct, color='darkred', linestyle='--', linewidth=2, alpha=0.7,
        ...
        ax3.plot(threshold_analysis['threshold'], threshold_analysis['affected_users'], 
                marker='^', linewidth=2, markersize=6, color='blue')
        ...
        ax4.plot(threshold_analysis['threshold'], threshold_analysis['affected_subreddits'], 
    ...
    def plot_subreddit_toxicity(self, threshold: float, subreddit_analysis: Dict):
        ...
        colors = ['#ffcccc', '#ff9999', '#ff6666', '#ff3333']  # Light to dark red gradient
        ...
        wedges, texts, autotexts = ax1.pie(sizes, 
                                          ...
                                          explode=[0.05 if p < 5.0 else 0 for p in percentages])  # Explode small slices
        ...
        ax1.set_title(f'Subreddit Toxicity Categories (Threshold: {threshold})', fontsize=12, pad=20)
        
        # Horizontal bar chart with value labels
        bars = ax2.barh(range(len(labels)), sizes, color=colors, alpha=0.8, edgecolor='black')
        ...
        for i, (bar, size, pct) in enumerate(zip(bars, sizes, percentages)):
            ...
            ax2.text(width + max(sizes)*0.01, bar.get_y() + bar.get_height()/2, 
                    f'{size} ({pct:.1f}%)', ha='left', va='center', fontweight='bold')
        
        plt.tight_layout()
        self.save_plot('subreddit_toxicity_analysis.png')
    
    def plot_top_users_subreddits(self, threshold: float, user_stats: pd.DataFrame, subreddit_stats: pd.DataFrame, top_n: int = 10):
        # Filter for meaningful data
        user_stats_filtered = user_stats[user_stats['total_comments'] > 3].head(top_n)
        subreddit_stats_filtered = subreddit_stats[subreddit_stats['total_comments'] > 10].head(top_n)
        ...
        if len(user_stats_filtered) > 0:
            ...
            ax1.barh(y_pos, user_stats_filtered['hate_ratio'], 
                    color='lightcoral', alpha=0.8, edgecolor='black')
            ax1.set_yticks(y_pos)
            ax1.set_yticklabels(user_stats_filtered.index)
            ax1.set_xlabel('Hate Ratio')
            ax1.set_title(f'Top {len(user_stats_filtered)} Users by Hate Ratio (>3 comments, threshold: {threshold})')
            ax1.grid(True, alpha=0.3, axis='x')
        
        # Top subreddits by hate ratio
        if len(subreddit_stats_filtered) > 0:
            y_pos = np.arange(len(subreddit_stats_filtered))
            ax2.barh(y_pos, subreddit_stats_filtered['hate_ratio'], 
                    color='lightblue', alpha=0.8, edgecolor='black')
            ax2.set_yticks(y_pos)
            ax2.set_yticklabels(subreddit_stats_filtered.index)
            ax2.set_xlabel('Hate Ratio')
            ax2.set_title(f'Top {len(subreddit_stats_filtered)} Subreddits by Hate Ratio (>10 comments, threshold: {threshold})')
            ax2.grid(True, alpha=0.3, axis='x')
    ...
    def plot_moderation_comparison(self, threshold: float, comparison: Dict):
        ...
        categories = ['Prediction\nFlagged', 'Ground Truth\nFlagged', 'Correctly\nFlagged', 
                     'Over-moderated\n(False Pos)', 'Under-moderated\n(False Neg)']
        values = [overall['prediction_flagged'], overall['ground_truth_flagged'], 
                 overall['correctly_flagged'], overall['over_moderated'], overall['under_moderated']]
        ...
        agreement_data = [overall['correctly_flagged'], 
                         overall['over_moderated'] + overall['under_moderated']]
        agreement_labels = ['Agreement', 'Disagreement']
        colors_pie = ['lightgreen', 'lightcoral']
        
        ax2.pie(agreement_data, labels=agreement_labels, autopct='%1.1f%%', 
               colors=colors_pie, startangle=90)
        ax2.set_title(f'Prediction vs Ground Truth Agreement\n(Comments, Threshold: {threshold})')
        
        plt.tight_layout()
        self.save_plot('moderation_comparison_analysis.png')
    
    def generate_all_visualizations(self, threshold: float, user_stats: pd.DataFrame, 
                                   subreddit_stats: pd.DataFrame, subreddit_analysis: Dict, 
                                   comparison: Dict, threshold_analysis=None):
