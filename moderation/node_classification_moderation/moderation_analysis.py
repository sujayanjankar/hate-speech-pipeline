...
class ModerationAnalyser:
    ...
    def analyze_user_behavior_patterns(self, threshold: float, user_stats: pd.DataFrame) -> Dict:
        no_toxicity = len(user_stats[user_stats['hate_ratio'] == 0])
        low_toxicity = len(user_stats[(user_stats['hate_ratio'] > 0) & (user_stats['hate_ratio'] <= 0.2)])
        medium_toxicity = len(user_stats[(user_stats['hate_ratio'] > 0.2) & (user_stats['hate_ratio'] <= 0.5)])
        high_toxicity = len(user_stats[(user_stats['hate_ratio'] > 0.5) & (user_stats['hate_ratio'] <= 0.8)])
        ...
        return {
            ...
            'user_stats_summary': {
                ...
                'users_with_violations': len(user_stats[user_stats['hate_predictions'] > 0])
            }
        }
    
    def analyze_subreddit_toxicity(self, threshold: float, subreddit_stats: pd.DataFrame) -> Dict:
        clean = len(subreddit_stats[subreddit_stats['hate_ratio'] == 0])
        low_toxicity = len(subreddit_stats[(subreddit_stats['hate_ratio'] > 0) & (subreddit_stats['hate_ratio'] <= 0.1)])
        medium_toxicity = len(subreddit_stats[(subreddit_stats['hate_ratio'] > 0.1) & (subreddit_stats['hate_ratio'] <= 0.3)])
        ...
        return {
            ...
            'subreddit_stats_summary': {
                ...
                'subreddits_with_violations': len(subreddit_stats[subreddit_stats['hate_predictions'] > 0])
            }
        }
    
    def analyze_moderation_efficiency(self, threshold: float, hate_comments: pd.DataFrame, bulk_result: Dict) -> Dict:
        ...
        coverage = bulk_result['comments_deleted'] / len(hate_comments) if len(hate_comments) > 0 else 0
        ...
        return {
            ...
            'efficiency_metrics': {
                'comments_per_banned_user': bulk_result['comments_deleted'] / max(bulk_result['users_banned'], 1),
                'violation_concentration': len(hate_comments) / hate_comments['author'].nunique() if len(hate_comments) > 0 else 0
            }
        }
    ...
    def analyze_moderation_comparison(self, threshold: float) -> Dict:
        ...
        over_moderated_users = pred_users - truth_users  # Users flagged by predictions only
        under_moderated_users = truth_users - pred_users  # Users flagged by ground truth only
        ...
        fp_analysis = {
            ...
            'users_affected': len(set(false_positives['author'])) if len(false_positives) > 0 else 0,
            'subreddits_affected': len(set(false_positives['subreddit'])) if len(false_positives) > 0 else 0,
            'avg_prediction_score': false_positives['gcn_prediction'].mean() if len(false_positives) > 0 else 0,
            'avg_ground_truth_score': false_positives['gcn_ground_truth'].mean() if len(false_positives) > 0 else 0,
            'score_difference': (false_positives['gcn_prediction'] - false_positives['gcn_ground_truth']).mean() if len(false_positives) > 0 else 0,
            'top_users': false_positives['author'].value_counts().head(5).to_dict() if len(false_positives) > 0 else {},
            'top_subreddits': false_positives['subreddit'].value_counts().head(5).to_dict() if len(false_positives) > 0 else {}
        }
        
        fn_analysis = {
            'count': len(false_negatives),
            'users_affected': len(set(false_negatives['author'])) if len(false_negatives) > 0 else 0,
            'subreddits_affected': len(set(false_negatives['subreddit'])) if len(false_negatives) > 0 else 0,
            'avg_prediction_score': false_negatives['gcn_prediction'].mean() if len(false_negatives) > 0 else 0,
            'avg_ground_truth_score': false_negatives['gcn_ground_truth'].mean() if len(false_negatives) > 0 else 0,
            'score_difference': (false_negatives['gcn_ground_truth'] - false_negatives['gcn_prediction']).mean() if len(false_negatives) > 0 else 0,
            'top_users': false_negatives['author'].value_counts().head(5).to_dict() if len(false_negatives) > 0 else {},
            'top_subreddits': false_negatives['subreddit'].value_counts().head(5).to_dict() if len(false_negatives) > 0 else {}
        }
        ...
        return {
            ...
            'overall_comparison': {
                ...
                'agreement_rate': len(true_positives) / max(len(pred_flagged), len(truth_flagged), 1)
            },
            'user_impact_comparison': {
                ...
                'user_agreement_rate': len(correctly_moderated_users) / max(len(pred_users), len(truth_users), 1)
            },
            'subreddit_impact_comparison': {
                ...
                'subreddit_agreement_rate': len(correctly_moderated_subreddits) / max(len(pred_subreddits), len(truth_subreddits), 1)
            },
            'false_positives_analysis': fp_analysis,
            'false_negatives_analysis': fn_analysis,
            'moderation_changes': {
                'would_not_moderate': list(over_moderated_users)[:10],  # Sample of users who wouldn't be moderated with ground truth
                'should_moderate': list(under_moderated_users)[:10],   # Sample of users who should be moderated with ground truth
                'over_moderated_subreddits': list(over_moderated_subreddits)[:10],
                'under_moderated_subreddits': list(under_moderated_subreddits)[:10]
            }
        }
    
    def generate_moderation_report(self, threshold: float, thresholds_for_impact: List[float], user_stats: pd.DataFrame, subreddit_stats: pd.DataFrame, hate_comments: pd.DataFrame, bulk_result: Dict) -> Dict:
        return {
            ...
            'subreddit_toxicity': self.analyze_subreddit_toxicity(threshold, subreddit_stats),
            'moderation_efficiency': self.analyze_moderation_efficiency(threshold, hate_comments, bulk_result),
