...
def show_basic_statistics(basic_data):
    ...
    print(f"Comments predicted as hate speech (>{HIGH_CONFIDENCE_THRESHOLD} probability): {basic_data['high_confidence_comments']}")
    print(f"Comments predicted as toxic (>{BASIC_TOXIC_THRESHOLD} probability): {basic_data['toxic_comments']}")
...
def show_top_subreddits(subreddit_stats):
    ...
    subreddit_stats_filtered = subreddit_stats[subreddit_stats['total_comments'] > MIN_SUBREDDIT_COMMENTS]
...
def show_moderation_recommendations(users_to_moderate, subreddits_to_moderate):
    ...
    if users_to_moderate:
        print(f"Users recommended for moderation (hate ratio >{USER_HATE_THRESHOLD}): {users_to_moderate[:5]}...")
    else:
        print(f"No users exceed the hate threshold of {USER_HATE_THRESHOLD}")
    
    if subreddits_to_moderate:
        print(f"Subreddits recommended for moderation (hate ratio >{SUBREDDIT_HATE_THRESHOLD}): {subreddits_to_moderate[:5]}...")
    else:
...
def show_comment_moderation_demo(bulk_demo_data):
    ...
    if bulk_demo_data:
        print(f"Found {bulk_demo_data['comment_count']} comments (>{bulk_demo_data['threshold_used']} threshold)")
        print("Processing...")
...
def show_prediction_analysis(dist_analysis):
    ...
    print(f"Score range: {dist_analysis['min_prediction']:.3f} - {dist_analysis['max_prediction']:.3f}")
...
def show_user_behavior_analysis(user_analysis):
    ...
    print(f"Users with violations: {user_analysis['user_stats_summary']['users_with_violations']}")
    print(f"Average toxic comment ratio: {user_analysis['user_stats_summary']['avg_toxic_comment_ratio']:.3f}")
...
def show_subreddit_analysis(subreddit_analysis):
    ...
    print(f"Subreddits with violations: {subreddit_analysis['subreddit_stats_summary']['subreddits_with_violations']}")
    print(f"Average toxic comment ratio: {subreddit_analysis['subreddit_stats_summary']['avg_toxic_comment_ratio']:.3f}")
...
def show_efficiency_analysis(efficiency_analysis):
    ...
    if efficiency_analysis.get('no_violations_found'):
        print(f"No violations found at threshold {efficiency_analysis['threshold_used']}")
        return
    
    print(f"Analysis threshold: {efficiency_analysis['threshold_used']}")
    print(f"Violations detected: {efficiency_analysis['violation_detection']['violations_found']}")
    print(f"Detection rate: {efficiency_analysis['violation_detection']['detection_rate']:.2f}%")
    print(f"Coverage rate: {efficiency_analysis['violation_detection']['coverage_rate']:.2f}%")
...
def show_moderation_comparison(comparison):
    ...
    print(f"\nSubreddit Impact Comparison:")
    print(f"  Subreddits affected by predictions: {subreddit_impact['prediction_subreddits']}")
    print(f"  Subreddits affected by ground truth: {subreddit_impact['ground_truth_subreddits']}")
    print(f"  Correctly identified subreddits: {subreddit_impact['correctly_moderated_subreddits']}")
    print(f"  Over-moderated subreddits: {subreddit_impact['over_moderated_subreddits']}")
    print(f"  Under-moderated subreddits: {subreddit_impact['under_moderated_subreddits']}")
    print(f"  Subreddit agreement rate: {subreddit_impact['subreddit_agreement_rate']:.2%}")
    ...
    if fp['count'] > 0:
        ...
        if fp['top_subreddits']:
            print(f"  Top over-moderated subreddits: {list(fp['top_subreddits'].keys())[:3]}")
    ...
    if fn['count'] > 0:
        ...
        if fn['top_subreddits']:
            print(f"  Top under-moderated subreddits: {list(fn['top_subreddits'].keys())[:3]}")
    
    # Moderation changes
    changes = comparison['moderation_changes']
    print(f"\nSpecific Moderation Changes:")
    if changes['would_not_moderate']:
        print(f"  Users who wouldn't be moderated with ground truth: {changes['would_not_moderate']}")
    if changes['should_moderate']:
        print(f"  Users who should be moderated with ground truth: {changes['should_moderate']}")
...
def show_analysis_initialised(overview):
    ...
    print(f"  Comments with hate downstream: {overview['dataset_summary']['hate_downstream_comments']:,}")
    print(f"  Threads analyzed: {overview['thread_structure']['total_threads_analyzed']:,}")
    print(f"  Actionable threads: {overview['thread_structure']['actionable_threads']:,}")
...
def show_complete_analysis_summary(complete_summary):
    ...
    print(f"  Hate downstream rate: {dataset_summary['hate_downstream_percentage']:.1f}%")
    ...
    print(f"  Actionable threads: {thread_structure['actionable_threads']:,} ({thread_structure['actionable_percentage']:.1f}%)")
...
def display_all_results(analysis_data):
    ...
    show_moderation_recommendations(analysis_data['users_to_moderate'], analysis_data['subreddits_to_moderate'])
